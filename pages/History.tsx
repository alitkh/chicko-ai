...
const History: React.FC = () => {
  return (
    <div className="p-6 max-w-2xl mx-auto pb-32 animate-[fadeIn_0.5s_ease-out]">
      ...
      {/* Search Bar - Use Flat variant for better performance */}
      <Glass className="rounded-xl px-4 py-3 flex items-center gap-3 mb-8" intensity="low" variant="flat">
      ...
      <div className="grid grid-cols-2 gap-4">
        {MOCK_HISTORY_IMAGES.map((src, i) => (
          <div key={i} className="aspect-[4/5] rounded-2xl overflow-hidden relative group cursor-pointer bg-white/5 transform-gpu">
            <img loading="lazy" src={src} alt={`History ${i}`} className="w-full h-full object-cover transition-transform duration-500 group-hover:scale-110" />
         ...
         {[1,2,3].map((_, i) => (
          <div key={`p-${i}`} className="aspect-[4/5] rounded-2xl bg-white/5 border border-white/5 flex items-center justify-center">
